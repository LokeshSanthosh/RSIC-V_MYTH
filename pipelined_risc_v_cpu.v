...
\TLV
   ...
   |cpu
      @0
         // 3-cycle RISC-V
         $reset = *reset;
         //$start = $reset ? 0 : (!$reset && >>1$reset ? 1 : 0);
         //$valid = $reset ? 0 : ($start ? 1 : >>3$valid);
         
         $pc[31:0] = >>1$reset ? 32'b0 : (>>3$valid_taken_br ? >>3$br_tgt_pc : (>>3$valid_load ? >>3$inc_pc : (>>3$valid_jump && >>3$is_jal ? >>3$br_tgt_pc : (>>3$valid_jump && >>3$is_jalr ? >>3$jalr_tgt_pc : >>1$inc_pc))));
         $imem_rd_en = !$reset;
